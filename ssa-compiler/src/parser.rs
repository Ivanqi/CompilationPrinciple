...
/// 
/// 这个函数执行两个主要步骤：
/// 1. 语法解析：使用LR解析器将源代码字符串解析为抽象语法树
/// 2. 语义验证：进行类型检查、变量绑定检查和作用域解析
/// 
/// # 参数
/// * `s` - 要解析的源代码字符串
/// 
/// # 返回值
/// * `Ok(Program)` - 如果解析和验证都成功，返回解析后的程序
/// * `Err(Box<dyn Error>)` - 如果解析或验证失败，返回详细的错误信息
pub fn parse_and_validate(s: &str) -> Result<Program, Box<dyn std::error::Error>> {
    // 第一步：语法解析
    // 调用parse函数进行词法分析和语法分析
    ...
    let program_unverified = match res {
        Some(Ok(p)) => p,  // 解析成功，获取程序AST
        Some(Err(e)) => return Err(format!("parse error: {}", e).into()),  // 解析错误
        None => return Err("unknown parse error".into()),  // 未知解析错误
    };
    
    // 第二步：语义验证
    // 调用validate函数进行类型检查、绑定解析等语义验证
